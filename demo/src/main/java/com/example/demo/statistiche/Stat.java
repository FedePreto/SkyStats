...
public class Stat {
		 ...
		 Convertitore conv = new Convertitore();
	        ...
	        try {
	        	...
	        	for(int i =0; i<c.size();i++){
	        		...
	        		if(c.get(i).getData().after(inizio) && c.get(i).getId()==idCitta && c.get(i).getData().before(fine)) {
	        			if(isPressione) {
	        	...
	        	for(int i =0; i<c.size();i++){
	        		...
	        		if(c.get(i).getData().after(inizio) && c.get(i).getNome().equals(citta) && c.get(i).getData().before(fine)){
	        			if(isPressione) {
	        				val.add(c.get(i).getPressione());	            		
	        			}else {
	        				val.add(c.get(i).getUmidita());
	...
	public void printMaxValues(Date inizio,Date fine) {
		...
		min[2]=getVarianza(getValues(inizio, fine, citta.get(0).getNome(), false), getMedia(getValues(inizio, fine, citta.get(0).getNome(), false)));
		min[3]=getVarianza(getValues(inizio, fine, citta.get(0).getNome(), true), getMedia(getValues(inizio, fine, citta.get(0).getNome(), true)));
		for(int i = 0; i<citta.size();i++) {
			//Max
			if( citta.get(i).getData().after(inizio) && citta.get(i).getUmidita()>max[0] && citta.get(i).getData().before(fine)) {
				max[0]=citta.get(i).getUmidita();
				maxU=i;
			}
			if( citta.get(i).getData().after(inizio) && citta.get(i).getPressione()>max[1] && citta.get(i).getData().before(fine)) {
				max[1] = citta.get(i).getPressione();
				maxP=i;
			}
			if(getVarianza(getValues(inizio, fine, citta.get(i).getNome(), false), getMedia(getValues(inizio, fine, citta.get(i).getNome(), false)))>max[2]) {
				max[2]=getVarianza(getValues(inizio, fine, citta.get(i).getNome(), false), getMedia(getValues(inizio, fine, citta.get(i).getNome(), false)));
				maxVU=i;
			}
			if(getVarianza(getValues(inizio, fine, citta.get(i).getNome(), true), getMedia(getValues(inizio, fine, citta.get(i).getNome(), true)))>max[3]) {
				max[3]=getVarianza(getValues(inizio, fine, citta.get(i).getNome(), true), getMedia(getValues(inizio, fine, citta.get(i).getNome(), true)));
				maxVP=i;
			}
		
			if(citta.get(i).getData().after(inizio) && citta.get(i).getUmidita()<min[0] && citta.get(i).getData().before(fine)) {
				min[0]=citta.get(i).getUmidita();
				minU=i;
			}
			if(citta.get(i).getData().after(inizio) && citta.get(i).getPressione()<min[1] && citta.get(i).getData().before(fine)) {
				min[1]=citta.get(i).getPressione();
				minP=i;
			}
			if(getVarianza(getValues(inizio, fine, citta.get(i).getNome(), false), getMedia(getValues(inizio, fine, citta.get(i).getNome(), false)))<min[2]) {
				min[2]=getVarianza(getValues(inizio, fine, citta.get(i).getNome(), false), getMedia(getValues(inizio, fine, citta.get(i).getNome(), false)));
				minVU=i;
			}
			if(getVarianza(getValues(inizio, fine, citta.get(i).getNome(), true), getMedia(getValues(inizio, fine, citta.get(i).getNome(), true)))<min[3]) {
				min[3]=getVarianza(getValues(inizio, fine, citta.get(i).getNome(), true), getMedia(getValues(inizio, fine, citta.get(i).getNome(), true)));
				minVP=i;
			}
		}
		System.out.println("La città con umidita maggiore è: " + citta.get(maxU).getNome() + " con " + max[0] + "%");
    	System.out.println("La città con pressione maggiore è: " + citta.get(maxP).getNome() + " con " + max[1] + " hPa");
    	System.out.println("La città con varianza di umidità maggiore è: " + citta.get(maxVU).getNome() + " con " + max[2]);
    	System.out.println("La citta con varianza di pressione maggiore è: "+citta.get(maxVP).getNome()+" con " + max[3]);
    	System.out.println();
    	...
    	System.out.println("La città con varianza di umidità minore è: " + citta.get(minVU).getNome() + " con " + min[2]);
    	System.out.println("La citta con varianza di pressione minore è: "+citta.get(minVP).getNome()+" con " + min[3]);
	...
	public void printMaxValues(Date inizio,Date fine) {
		...
		for(int i = 0; i<citta.size();i++) {
			//Max
			if(citta.get(i).getUmidita()>=citta.get(maxU).getUmidita()&& citta.get(i).getData().before(fine) && citta.get(i).getData().after(inizio)) {
				maxU=i;
			}
			if(citta.get(i).getPressione()>citta.get(maxP).getPressione()&& citta.get(i).getData().before(fine) && citta.get(i).getData().after(inizio)) {
				maxP=i;
			}
			...
			if(getVarianza(getValues(inizio, fine, citta.get(i).getNome(), true), getMedia(getValues(inizio, fine, citta.get(i).getNome(), true)))<getVarianza(getValues(inizio, fine, citta.get(minVP).getNome(), true), getMedia(getValues(inizio, fine, citta.get(minVP).getNome(), true)))) {
				minVP=i;
			}
		}
