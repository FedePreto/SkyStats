...
public class Statistiche {
	...
	public static double calcolaVarianzaUmidità(Date inizio, Date fine, int idCitta) {
		...
		// Da fare con le collection ma siccome non le so ancora usare bene lo faccio
		// con gli arraylist
		ArrayList<Double[]> temp = new ArrayList<Double[]>();

		double media = calcolaMediaUmidita(inizio, fine, idCitta);// media umidita
		for (int i = 0; i < c.size(); i++) {// cicla tutti i risulati
			if (c.get(i).getId() == idCitta && c.get(i).getData().before(fine) && c.get(i).getData().after(inizio)) {// se
																														// il
																														// risultato
																														// corrisponde
																														// con
